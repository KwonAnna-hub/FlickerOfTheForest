...
{
    private Transform treeTrans; // 나무 객체
    private GameObject player; // 플레이어 객체
    private Camera assignedCamera; //카메라라
    private Transform playerTrans;
    private Transform agentTrans;
    private Rigidbody agent_Rigidbody;
    Vector3 treeInitPos;

    public Transform rotateTrans;//방향 설정 오브젝트
    ...
    public override void Initialize(){//스타트함수와 거의 유사
        ...
        treeTrans = GameObject.Find("Tree").transform;
        player = GameObject.Find("Player");
        groundCheck = GameObject.Find("Terrain_Center").transform;
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if(rotateTrans != null && moveDirection != Vector3.zero){
            RotateObject(moveDirection);
        }
    }

    private void RotateObject(Vector3 moveDirection){
        Quaternion targetRotation = Quaternion.LookRotation(moveDirection);
        rotateTrans.rotation = Quaternion.Slerp(rotateTrans.rotation, targetRotation, Time.deltaTime * RotationSpeed);
    }

    private void OnCollisionEnter(Collision collision) {
        foreach (Collider childCollider in treeTrans.GetComponentsInChildren<Collider>()){
            if(collision.collider == childCollider){
                TreeHealth treeHealth = treeTrans.GetComponent<TreeHealth>();
                if(treeHealth != null && !isDamaging){
                    StartCoroutine(DealDamage(treeHealth));
                }
            }
        }
    }

    private IEnumerator DealDamage(TreeHealth treeHealth){
        isDamaging = true;
